...
@section('page-title', 'Cash Withdrawal')
@section('page-subtitle') <a href=" {{url('/home')}}">Dashboard</a>@endsection
@section('subtitle', 'Cash Withdrawal')
@section('content')
    <div class="row">
        <div class="col-12">
            <div class="card">
                    ...
                    <form action="/search_user" method="get">
                        ...
                        <div class="mb-3 float-right">
                            <button type="button" class="btn btn-primary btn-md"
                                    onclick="window.location='{{ url('/CashWithdrawal')}}'">
                                ...
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        <form role="form" id="createform"
                                              action="{{action('WithdrawalController@store')}}"
                                            ...
                                            <div class="card-body">
                                                ...
                                                <div class="form-group">
                                                    <label for="chart_account_id">Cash Withdrawal</label>
                                                    <select class="custom-select" name="chart_account_id" required>
                            ...
                            <div class="table-responsive-sm">
                                <table id="example2" class="table table-bordered table-hover">
                                    ...
                                    <tr>
                                        ...
                                        <th>Branch Name</th>
                                        <th>Cash Withdrawal Description</th>
                                    ...
                                    <tbody>
                                    <?php $no = 1; ?>
                                    @foreach ($cashWithdrawals as $cashWithdrawal)
                                        <tr role="row" class="odd">
                                            <td>{{$no++}}</td>
                                            <td>{{$cashWithdrawal -> branchs -> branch_name}}</td>
                                            <td>{{$cashWithdrawal -> chartAccounts -> chart_account_name_kh}}</td>
                                            <td>{{$cashWithdrawal -> date}}</td>
                                            <td>{{$cashWithdrawal -> currency}}</td>
                                            <td>{{$cashWithdrawal -> amount}}</td>
                                            <td>{{$cashWithdrawal -> exchange_rate}}</td>
                                            <td>{{$cashWithdrawal -> description}}</td>
                                            <td>
                                                <a href="{{ route('CashWithdrawal.edit',$cashWithdrawal->cash_withdrawal_id) }}"
                                                   class="btn btn-primary btn-sm"> <i
                                                        class="fa fa-edit"></i></a>
                                                <button type="button" class="btn btn-danger btn-sm btn_delete"
                                                        name="btnDelete"
                                                        onclick="delete_CashWithdrawal({{$cashWithdrawal->cash_withdrawal_id}})">
        ...
        <script type="text/javascript">
            ...
            function delete_CashWithdrawal(id) {
                Swal.fire({
                    title: 'Are you sure?',
                                    ...
                                    .then((willDelete) => {
                                        if (willDelete) {
                                            if (result.status == 200) {
                                                window.location.href = "{{url('CashWithdrawal')}}";
                                            }
                                        }
                                    })
                            }
