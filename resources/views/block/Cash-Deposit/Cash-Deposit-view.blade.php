...
@section('page-title', 'Cash Deposit')
@section('page-subtitle') <a href=" {{url('/home')}}">Dashboard</a>@endsection
@section('subtitle', 'Cash Deposit')
@section('content')
    <div class="row">
        <div class="col-12">
            <div class="card">
                    ...
                    <form action="/search_user" method="get">
                        ...
                        <div class="mb-3 float-right">
                            <button type="button" class="btn btn-primary btn-md"
                                    onclick="window.location='{{ url('/CashDeposit')}}'">
                                ...
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        <form role="form" id="createform"
                                              action="{{action('CashDepositController@store')}}"
                                            ...
                                            <div class="card-body">
                                                ...
                                                <div class="form-group">
                                                    <label for="chart_account_id">Cash Deposit</label>
                            ...
                            <div class="table-responsive-sm">
                                <table id="example2" class="table table-bordered table-hover">
                                    ...
                                    <tr>
                                        ...
                                        <th>Branch Name</th>
                                        <th>Cash Deposit Description</th>
                                    ...
                                    @foreach ($cashDeposits as $cashDeposit)
                                        <tr role="row" class="odd">
                                            <td>{{$no++}}</td>
                                            <td>{{$cashDeposit -> branchs -> branch_name}}</td>
                                            <td>{{$cashDeposit -> chartAccounts -> chart_account_name_kh}}</td>
                                            <td>{{$cashDeposit -> date}}</td>
                                            <td>{{$cashDeposit -> currency}}</td>
                                            <td>{{$cashDeposit -> amount}}</td>
                                            <td>{{$cashDeposit -> exchange_rate}}</td>
                                            <td>{{$cashDeposit -> description}}</td>
                                            <td>
                                                <a href="{{ route('CashDeposit.edit',$cashDeposit->cash_deposit_id) }}"
                                                   class="btn btn-primary btn-sm"> <i
                                                        class="fa fa-edit"></i></a>
                                                <button type="button" class="btn btn-danger btn-sm btn_delete"
                                                        name="btnDelete"
                                                        onclick="delete_OtherIncome({{$cashDeposit->cash_deposit_id}})">
                                    ...
                                    .then((willDelete) => {
                                        if (willDelete) {
                                            if (result.status == 200) {
                                                window.location.href = "{{url('CashDeposit')}}";
                                            }
                                        }
                                    })
                            }
