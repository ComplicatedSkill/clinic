...
@section('content')
    ...
    <div class="col-md-12">
        <div class="card card-blue">
            ...
            <!-- /.card-header -->
            <div class="card-body">
                <form role="form" action="{{action('AppointmentController@update',$appointments->appointment_id)}}"
                        ...
                        <div class="col-sm-6">
                            <div class="form-group">
                                ...
                                <input class="form-control" id="appointment_tittle" name="appointment_tittle"
                                       placeholder="Appointment Tittle" value="{{$appointments->title}}"
                                       type="text" required/>
                            </div>
                        </div>
                        <div class="col-sm-6">
                            ...
                            <div class="form-group">
                                ...
                                <div class="input-group my-colorpicker2">
                                    <input type="text" class="form-control" name="color" placeholder="Appointment Color"
                                           value="{{$appointments->color}}" required>
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control select2" name="patient_id" required>
                                    @foreach ($patients as $patient)
                                        <option
                                            value="{{$patient->patient_id}}" {{$patient->patient_id=$appointments->patient_id?'selected':''}}>{{$patient->patient_name_kh}}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control select2" name="staff_id" required>
                                    @foreach ($staffs as $staff)
                                        <option
                                            value="{{$staff->staff_id}}" {{$staff->staff_id=$appointments->staff_id?'selected':''}}>{{$staff->staff_name_kh}}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <select class="custom-select" name="branch_id" id="branch_id" required>
                                    @foreach ($branchs as $branch)
                                        <option
                                            value="{{$branch->branch_id}}" {{$branch->branch_id=$appointments->branch_id?'selected':''}}>{{$branch->branch_name}}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <select class="custom-select" name="department_id" required>
                                    @foreach ($departments as $department)
                                        <option
                                            value="{{$department->department_id}}" {{$department->department_id=$appointments->department_id?'selected':''}}>{{$department->department_name}}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                        ...
                        <div class="col-sm-6">
                            <div class="form-group">
                                ...
                                <input data-format="dd/MM/yyyy hh:mm:ss" class="form-control" id="date"
                                       name="date" placeholder="Appointment Date" value="{{$appointments->date}}"
                                       type="text" required/>
                            </div>
                        </div>
                        <div class="col-sm-6">
                            <div class="form-group">
                                ...
                                <div class="input-group date" id="timepicker" data-target-input="nearest">
                                    <input type="text" class="form-control datetimepicker-input"
                                           ...
                                           placeholder="Appointment Time" name="time" value="{{$appointments->time}}"
                                           required/>
                                </div>
                                <!-- /.input group -->
                            </div>
                    ...
                    <div class="col-sm-12">
                        <div class="form-group">
                            ...
                            <textarea class="form-control" rows="5" placeholder="note" id="note"
                                      name="note">{{$appointments->note}}</textarea>
                        </div>
                    </div>
                    ...
                    <div class="col-2" style="float: right">
                        <button type="submit" class="btn btn-primary btn-block btn-flat">Update Appointment</button>
                    </div>
                    <div class="col-2" style="float: right">
                        <button type="button" class="btn btn-danger btn-block btn-flat"
                                onclick="delete_appointment({{$appointments-> appointment_id}})">
                            Delete Appointment
                        </button>
                    </div>
                </form>
